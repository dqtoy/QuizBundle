    ...
    public class QuestionsData : DataModel
    {
        ...
        void Awake()
        {
			...
			if (FindObjectOfType<ConfigQuizController>() != null)
				ConfigQuizController.Instance.SimpleBoolEvent += ChangeRandomValue;
			
		}

		public void ChangeRandomValue(bool value) 
		{
        ...
        protected override void OnStart()
        {
			...
			if(ConfigQuizController.Instance != null)
				ConfigQuizController.Instance.SimpleSetQuizEvent += SetQuestionsAtt;
			//JsonConfig.Instance.DataNameQuestions = DataFile;
			
			base.OnStart();
        }

		public void SetQuestionsAtt(List<Question> question) 
		{
			Questions = new List<Question>();
			ConfigQuizController.Instance._Questions = new List<Question>();

			for (int i = 0; i< question.Count; i++) 
			{
				Questions.Add(question[i]);
				ConfigQuizController.Instance._Questions.Add(question[i]);
			}

			//JsonConfig.Instance.SaveQuestions();
			JsonConfig.Instance.Save<List<Question>>(DataFile, Questions);
        ...
        public void NextQuestion(int type)
        {
			...
			// E AQUI QUE ELE SET TODOS AS REFERENCIAS TEXTOS ETC E FAZ UPDATE
