...
namespace InterativaSystem.Controllers
{
    ...
    public class QuizController : GenericController
    {
		...
		// Jonathan Cod
		public UnityEngine.UI.Text txtCurrentP;  
		...
		[HideInInspector] int limitPerTypes,qtdLimit;
		[HideInInspector] List<int> myTypes;
        [HideInInspector]
        ...
        void Awake()
        {
			...
			if (FindObjectOfType<ConfigQuizController>() != null)
				ConfigQuizController.Instance.SimpleIntEvent += ChangeValueLimit;
        ...
        protected override void OnStart()
        {
			...
			/*_timeController.AddQuestionRight += () => {
				QuestionsRight++;
		...
		public void ChangeValueLimit(int value)
		{
			QuestionLimit = value;
		}
        ...
        public override void EndGame()
        {
			...
			//LOGICA PARA RANKING QUIZ JONATHAN
			//if (FindObjectOfType<UserController>() != null)
			//{
			//	UserController.Instance.SetInfo(QuestionsRight, _timeController.TimeSinceGameStart);
			//	ScreenGameRequest.Instance.PostUser();
			//}


			//CallAction(QuestionsRight >= 4 ? 1 : 0);

			Debug.Log(QuestionsRight);

			if(QuestionsRight > 4) { CallAction(1); }
			else if(QuestionsRight >= 3) { CallAction(2); }
			else   { CallAction(0); }
		...
		public void PrepareGame(List<int> types)
		{
			base.PrepareGame();
			...
			limitPerTypes = QuestionLimit / types.Count;
			qtdLimit = -1;
			CallNextQuestion();
		}
		...
		public void CallNextQuestion()
        {
			...
			if (qtdLimit / limitPerTypes == 1 && myTypes.Count != 1)
			{
				qtdLimit = 0;
				...
				int index = Random.Range(0, myTypes.Count);
				_type = myTypes[index];

				Debug.Log("==typeNew " + _type);
			}


			txtCurrentP.text = (_questionCount + 1).ToString() + " / ";
